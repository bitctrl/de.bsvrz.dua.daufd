...
/**
 * Generiert zufaellige hystereze Intervalle, dann zufaellige Werte, diese
 * Klassifiziert und vergleicht die Antwort der Hystereze Klasse
 *
...
public class HysterezeTester2 {
	...
	public void init(final double[] von, final double[] bis) {
		stufeVon = new double[von.length];
		stufeBis = new double[bis.length];
		for (int i = 0; i < (von.length - 1); i++) {
			Assert.assertTrue("Naechste Stufe beginnt nach luecke: [" + von[i]
					+ "," + bis[i] + "] naechste von: " + von[i + 1],
					von[i + 1] <= bis[i]);
			Assert.assertTrue("Naechste Stufe ueberdeckt die Vorherige: ["
					+ von[i] + "," + bis[i] + "] naechste von: " + von[i + 1],
					von[i + 1] > von[i]);
		}
	...
	public void Test() {
		...
		for (int i = 0; i < 50; i++) {
			...
			final int intervalle = 2 + (int) (Math.random() * 18);

			final double[] vonMenge = new double[intervalle];
			final double[] bisMenge = new double[intervalle];
			...
			for (int j = 0; j < (intervalle - 1); j++) {
				bisMenge[j] = vonMenge[j]
						+ (Math.random() * (maxWert - vonMenge[j]));
				vonMenge[j + 1] = vonMenge[j]
						+ (Math.random() * (bisMenge[j] - vonMenge[j]));
			}

			final Hysterese hyst = new Hysterese();
			try {
			...
			for (int k = 0; k < ANZAHL_TESTS; k++) {
				...
				final int r2 = this.hystereze(rand, alt);

				Assert.assertTrue(r1 == r2);

				System.out.println(String.format(
						"[ %6d ] Test bestanden: %2d == %2d", anzahlTest++, r1,
						r2));
				alt = r2;
			}
		}
